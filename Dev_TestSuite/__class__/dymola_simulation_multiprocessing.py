...
class DymolaSimulation(object):
    ...
    def __init__(self,libs,dymola_egg,dymola_launcher,reload=False):
        """
        ...
        self.init_paramsDico = {'libs' : libs,  'dymola_egg' : dymola_egg, 'dymola_launcher' : dymola_launcher}
        ...
        # through a subprocess (BaseManager module)
        BaseManager.register('DymolaInterface', DymolaInterface)
        manager = BaseManager()
        manager.start()
        try:
            self.inst = manager.DymolaInterface(dymolapath=self.init_paramsDico['dymola_launcher'])
            print("dymola interface object created through BaseManager..\n")
        except ProcessError as err:
            print(f'__init__ -> error : {err}')

        # Load Modelica Libraries
        try:
            for library in self.init_paramsDico['libs']:
                self.inst.openModel(library)
...
###############################################################################
    ...
    def gen_timerDico(self):
        ...
        try:
            ...
            self.timerDico[f'timeout ({self.timeout_fix}s)'] = False
            if self.isProcessKilled:
                self.timerDico[f'timeout ({self.timeout_fix}s)'] = True
...
###############################################################################
    ...
    def referencefileGeneration(self, log=0):
        ...
        referenceFileDirectory = path.join(repo_path,f'devtest_thermosyspro/Dev_TestSuite/data/referencefile_results/{self.get_class_name()}').replace('\\', '/')
        
        # Old one
        # returnsDico['command_result'] = self.inst.ExecuteCommand(f'ModelManagement.Check.checkLibrary(false, false, false, false, "{self.get_class_name()}", generateReference=true, referenceFileDirectory="C:/Users/H13483/Documents/devtest_thermosyspro/Dev_TestSuite/data/referencefile_results/{self.get_class_name()}")')
        
        returnsDico['command_result'] = self.inst.ExecuteCommand(f'ModelManagement.Check.checkLibrary(false, false, false, false, "{self.get_class_name()}", generateReference=true, referenceFileDirectory="{referenceFileDirectory}")')

        
        # Log saving
        lastlog = self.inst.getLastErrorLog()
        if lastlog:
            try:
    ...
    def check(self, log=0):
        ...
        result = self.inst.checkModel(self.get_class_name())
    ...
    def translate(self, log=0):
        ...
        result = self.inst.translateModel(self.get_class_name())
    ...
    def simulate(self, log=0):
        ...
        # Initializaion
        try:
            self.class_text = self.inst.getClassText(self.get_class_name(), includeAnnotations=True)[0]
        ...
        self.isProcessKilled = False
        self.timeout_fix = 120 # 2 minutes countdown
        
        # (in development)
        # timeout_relative = self.get_timeout_relative()
        
        queue = Queue()
        
        try:
            simProcess = Process(target=process_helper, args=[self.inst, queue, self.get_class_name(), self.model_name, self.stop_time])
            simProcess.start()
            print("process start...")
            print("model simulation...")
            simProcess.join(self.timeout_fix)
        except ProcessError as err:
            print(f'simulate -> ProcessError : {err}')
            
        # Prints
        # print(f"-- PRINT -- \ndymola_simulation_multiprocessing.py main -> simProcess.is_alive() -> {simProcess.is_alive()}\n")
        
        if simProcess.is_alive():
            simProcess.terminate()
            print('Process killed!')
            self.isProcessKilled = True
            result = False
            
            # Reload the dymolainterface instance
            __init__argsDico = self.init_paramsDico.copy()
            __init__argsDico['reload'] = True
            self.__init__(**__init__argsDico)
...
def process_helper(inst, queue, class_name, model_name, stop_time):
    # Prints - doesn't show up in the main console
    # print(f"-- PRINT -- \ndymola_simulation_multiprocessing.py process_helper -> inst -> {inst}\n")
    # print(f"-- PRINT -- \ndymola_simulation_multiprocessing.py process_helper -> queue -> {queue}\n")
    # print(f"-- PRINT -- \ndymola_simulation_multiprocessing.py process_helper -> class_name -> {class_name}\n")
    # print(f"-- PRINT -- \ndymola_simulation_multiprocessing.py process_helper -> model_name -> {model_name}\n")
    try:
        result = inst.simulateModel(f'{class_name}', resultFile=f'{model_name}', stopTime=stop_time)
    except Exception as err:
        print(f'process_helper -> Error : {err}')
        result = None
    try:
        queue.put(result)
    except Exception as err:
...
if __name__ == '__main__':
    print('main : dymola_simulation_multiprocessing')

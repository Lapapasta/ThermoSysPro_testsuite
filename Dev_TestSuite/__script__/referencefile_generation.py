...
def main():
    ...
    try:
        os.mkdir('referencefile_results')
    except OSError as err:
        print(f'OSError : {err}')
        print('still progress..')
    
    # Reception directories for simulation results
    os.chdir(os.path.join(path.join(repo_path,r'devtest_thermosyspro\Dev_TestSuite\data'),'referencefile_results'))
    ...
    # Models reference file generation
    referencefileResultsDico = DymolaInstance.severalReferencefileGeneration(IO_Manager.model4simu)

    # Reference file generation results reception
    IO_Manager.setParams(referencefileResultsDico=referencefileResultsDico)
    
    # print(f"-- PRINT -- \nreferencefile_generation.py main -> IO_Manager.paramsDico['referencefileResultsDico'] -> {IO_Manager.paramsDico['referencefileResultsDico']} \n")
   
    # Export reference file generation results into a .csv file
    if IO_Manager.paramsDico['referencefileResultsDico']['results']:
         IO_Manager.csvCall(call='referencefile')
